...
import type { PaginationOrders } from '@/types/order'
defineProps<{
...
const { formState, getOrders, reload } = useOrderStore()
...
<template>
  <main class="flex-1">
      ...
      <div class="flex-1 min-w-0">
        <div class="sm:flex-auto">
          ...
          <p class="mt-2 text-sm text-gray-700">
            A list of all the orders in the system including their oder number and status.
          </p>
        </div>
      </div>
      <div class="flex justify-between gap-4 mt-4 sm:justify-start sm:mt-0 sm:ml-4">
        <button
          v-if="useGate().can('order-create')"
    ...
    <div class="grid items-end grid-cols-1 gap-5 px-4 mb-4 sm:grid-cols-12 sm:px-6 lg:px-8">
      ...
      <div class="order-last col-span-12 sm:col-span-7 sm:order-first">
        <p class="text-sm text-gray-700">
          ...
          {{ orders.meta.total > 0 ? 'results' : 'result' }}
        </p>
      </div>
    ...
    <!-- Slide over -->
    <AdminOrdersSlideOver :state="formState.show" />
  </main>
</template>
