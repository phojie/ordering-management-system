              ...
              <div>
                ...
                <div class="relative -ml-2 bg-gray-900 border-transparent rounded-md group focus-within:ring-2 focus-within:ring-white">
                  ...
                  <div class="absolute inset-y-0 right-0 flex items-center pointer-events-none">
                    <heroicons-chevron-down class="w-5 h-5 text-gray-300" aria-hidden="true" />
                  </div>
                </div>
              </div>
            </form>
                  ...
                  <div class="flex justify-center h-full space-x-8">
                    <Popover v-for="category in navigation.categories" :key="category.name" v-slot="{ open }" class="flex">
                      ...
                      <transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-150 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
                        <PopoverPanel class="absolute inset-x-0 z-10 text-sm text-gray-500 bg-white top-full">
                          ...
                          <div class="absolute inset-0 top-0 h-px px-8 mx-auto max-w-7xl" aria-hidden="true">
                            <div class="w-full h-px transition-colors duration-200 ease-out" :class="[open ? 'bg-gray-200' : 'bg-transparent']" />
                          </div>

                          <div class="relative">
                            <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
                              <div class="grid grid-cols-4 py-16 gap-y-10 gap-x-8">
                                <div v-for="item in category.featured" :key="item.name" class="relative group">
                                  <div class="overflow-hidden bg-gray-100 rounded-md aspect-w-1 aspect-h-1 group-hover:opacity-75">
                                    <img :src="item.imageSrc" :alt="item.imageAlt" class="object-cover object-center">
                                  </div>
                                  <a :href="item.href" class="block mt-4 font-medium text-gray-900">
                ...
                <div class="flex items-center lg:ml-8">
                  ...
                  <div class="flow-root ml-4 lg:ml-8">
                    <a href="#" class="flex items-center p-2 -m-2 group">
                      <heroicons-shopping-bag class="flex-shrink-0 w-6 h-6 text-gray-400 group-hover:text-gray-500" aria-hidden="true" />
