<script setup lang="ts">
const props = defineProps<{
  ordersProductQuantity: {
    categories: Array<any>
    series: Array<any>
  }
}>()
</script>

<template>
  <div>
    <h2 class="mb-5 text-lg font-medium leading-6 text-gray-900">
      Overview
    </h2>
    <div class="grid flex-1 grid-cols-1 gap-4 lg:grid-cols-12">
      <div class="col-span-1 p-2 bg-white rounded-lg shadow lg:col-span-12 ">
        <ChartBar :series="ordersProductQuantity.series" :categories="ordersProductQuantity.categories" />
      </div>
      <!-- <div class="col-span-1 p-2 bg-white rounded-lg shadow lg:col-span-4 min-h-[500px]">
        <ChartSpline />
      </div> -->
    </div>
  </div>
</template>
